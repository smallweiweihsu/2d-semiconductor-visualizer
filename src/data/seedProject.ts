import { libraryPapers } from './paperLibrary'
import type {
  DeviceLayer,
  DeviceLayerRole,
  DeviceStructure,
...
export const materials: Material[] = [
  material('wse2', 'WSe2', 'WSe₂', 'two_d_semiconductor', 'p', '二維過渡金屬硫族化物，常用於場效電晶體', materialColors.wse2, estimated('1.2-1.65', 'eV', '塊材間接 ~1.2、單層直接 ~1.65 eV'), estimated(3.9, 'eV', '常見 ~3.9–4.0 eV'), estimated('4.0-4.5', 'eV', '層數依賴（1–10L），未含摻雜'), estimated(7.5, '', '面外介電常數約 7–8 (Laturia 2018)'), estimated(80, 'cm²/V·s', '隨製程/接觸變動大'), unknown('Ω·m'), estimated(3.28, 'Å'), estimated(0.7, 'nm'), ['層數、缺陷、接觸金屬與製程會強烈影響電性']),
  material('mos2', 'MoS2', 'MoS₂', 'two_d_semiconductor', 'n', '常見二維 TMD 半導體', materialColors.mos2, range('1.2-1.9', 'eV', '塊材間接 ~1.2、單層直接 ~1.8 eV'), range('4.0-4.3', 'eV'), estimated('4.5-5.0', 'eV', '層數依賴（1–10L），未含摻雜'), estimated(7, '', '面外 ~6.9–7.6 (Laturia 2018)'), range('1-200', 'cm²/V·s'), unknown('Ω·m'), estimated(3.16, 'Å'), estimated(0.65, 'nm'), ['二維 TMD 代表材料']),
  ...
  material('graphene', 'Graphene', 'Graphene', 'metal', 'unknown', '二維半金屬，常作為 vdW 接觸電極', '#64748b', known(0, 'eV', '半金屬，無能隙'), known('—', 'eV', '半金屬不適用'), estimated(4.5, 'eV', '~4.5 eV，可由摻雜/閘極調變'), known('—', '', '不適用'), estimated(10000, 'cm²/V·s', '高遷移率，依品質'), estimated(1e-6, 'Ω·m', '面內近似'), estimated(2.46, 'Å', '晶格常數'), estimated(0.34, 'nm', '單層'), ['二維半金屬，常作 vdW 接觸電極，功函數可調']),
  material('inse', 'InSe', 'InSe', 'two_d_semiconductor', 'n', '二維 III–VI 半導體，能隙隨層數變化', '#34d399', range('1.2-2.6', 'eV', '塊材 ~1.2、單層 ~2.6 eV'), estimated(4.6, 'eV', '約 4.5–4.6 eV'), estimated('4.5-4.9', 'eV', '層數依賴（1–10L），未含摻雜'), estimated(7.6, '', '~7–10'), range('10-1000', 'cm²/V·s'), unknown('Ω·m'), estimated(4.0, 'Å'), estimated(0.8, 'nm'), ['層數強烈影響能隙與電子親和力（需文獻校準）']),
]
...
export const seedProject: SemivizProject = {
  ...
  references: [
    ...
    { id: 'lit-001', title: 'Epitaxially grown single-crystalline antimony trioxide dielectrics for two-dimensional electronics', authors: 'Nat. Electron. 2026 作者群', year: 2026, journal: 'Nature Electronics', material: 'WSe2, Sb2O3', electrode: 'Sb₂O₃ 頂閘', doi: '10.1038/s41928-026-01580-w', parameterExtracted: 'SS, Dit, k, breakdown', reliabilityScore: 9, status: 'accepted', notes: '單晶 Sb₂O₃ 介電應用於二維電子（Nature Electronics 2026）：以 vdW 磊晶在二維半導體上成長單晶 Sb₂O₃。單層 Sb₂O₃ 介電常數約 6、崩潰電場約 11 MV/cm、介面缺陷密度低至 3.8×10¹⁰ cm⁻²eV⁻¹。以單層 Sb₂O₃/雙層 WSe₂ 頂閘 FET 達 SS 62 mV/dec、on/off 10⁶、低閘漏 10⁻⁴ A/cm²，並做出 60 顆陣列（95% 良率）。與你 Sb₂O₃ 頂閘 WSe₂ 主題完全吻合。★Sb₂O₃ 頂閘。' },
    { id: 'lit-002', title: 'High-Performance Single Layered WSe2 p-FETs with Chemically Doped Contacts', authors: 'H. Fang, A. Javey et al. (UC Berkeley)', year: 2012, journal: 'Nano Letters', material: 'WSe2, Pd', electrode: 'Pd', doi: '10.1021/nl301702r', parameterExtracted: 'µ, Rc, on/off', reliabilityScore: 9, status: 'accepted', notes: '高效能單層 WSe₂ p-FET 與化學摻雜接觸（Nano Lett. 2012, Fang & Javey, Berkeley）：以高功函數 Pd 作為電洞接觸，並以 NO₂ 表面摻雜降低接觸電阻，實現電洞遷移率約 250 cm²/V·s、近理想次臨界擺幅與高 on/off 的 p 型 WSe₂ FET。是 WSe₂ p-FET 與接觸工程的奠基論文，對你 Pd/WSe₂ 接觸選擇與 p 型策略很有參考。★Pd 接觸。' },
    { id: 'lit-003', title: 'Self-Limiting Layer-by-Layer Oxidation of Atomically Thin WSe2', authors: 'M. Yamamoto, K. Tsukagoshi et al. (NIMS)', year: 2015, journal: 'Nano Letters', material: 'WSe2, WOx', electrode: '氧化/WOx', doi: '10.1021/nl5049753', parameterExtracted: 'oxidation, layer control', reliabilityScore: 7, status: 'reviewed', notes: '原子級薄 WSe₂ 的自限層狀氧化（Nano Lett. 2015, Yamamoto/Tsukagoshi, NIMS）：100°C 以下臭氧處理使 WOx 沿硒鋸齒邊側向成長，氧化區合併後自動終止，呈自限性；較高溫可逐層氧化至三層。提供以可控氧化「削層」與形成均勻 WOx 的方法，對你 WSe₂ 表面氧化／WOx 介面工程與層數控制具直接參考。★氧化/WOx。' },
    { id: 'lit-004', title: 'Approaching the Schottky-Mott limit in van der Waals metal-semiconductor junctions', authors: 'Y. Liu, X. Duan et al. (UCLA)', year: 2018, journal: 'Nature', material: 'MoS2, In', electrode: 'In (vdW)', doi: '10.1038/s41586-018-0129-8', parameterExtracted: 'S-parameter, Rc', reliabilityScore: 10, status: 'accepted', notes: '逼近 Schottky–Mott 極限的 vdW 金屬–半導體接面（Nature 2018, Liu et al., UCLA）：將原子級平整金屬薄膜（Au 包覆 In）轉印貼合二維半導體，避免化學鍵結與費米釘扎，介面 S 參數高達 0.96，為當時最接近理想極限者，並得到低接觸電阻。是「低損傷 In 轉印接觸、解除釘扎」的代表論文，與你接觸／能帶設計高度相關。★In 接觸/能帶。' },
    { id: 'lit-005', title: 'Electrodes for High-k Molecular Crystal Antimony Trioxide Gate Dielectrics for 2D Electronics', authors: 'S. Ranjan et al.', year: 2024, journal: 'Adv. Electron. Mater.', material: 'Sb2O3', electrode: '介電/電極', doi: '10.1002/aelm.202400205', parameterExtracted: 'k, leakage', reliabilityScore: 7, status: 'candidate', notes: '高-κ 分子晶體 Sb₂O₃ 閘介電之電極研究（Adv. Electron. Mater. 2024, Ranjan et al.）：探討在 Sb₂O₃ 閘介電上搭配不同電極（功函數／介面）對二維電晶體閘控、漏電與可靠度的影響，提供 Sb₂O₃ 作為高-κ 介電整合時的電極選擇依據。與你 Sb₂O₃ 介電＋上電極（Pd/Sb 等）整合直接相關。★Sb₂O₃ 介電/電極。' },
    ...libraryPapers,
