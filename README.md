...
## Batch 16：錯誤修復、物理機制補齊與版面簡化

### 修復

- 修復「氧化模擬」模組未掛載到任何分頁、整個模組無法進入的問題（重新加入氧化模擬分頁）。
- 新增 Workspace 錯誤邊界：單一分頁發生 runtime 錯誤時只顯示錯誤卡片並可重試，不會整頁空白或卡住分頁切換。
- `vite.config.ts` 加入 `base: './'`，`npm run build` 後的 `dist/index.html` 可直接用瀏覽器開啟，也可部署在子路徑（如 GitHub Pages）。
- 支援瀏覽器上一頁 / 下一頁與手動修改網址 hash 切換分頁（hashchange 監聽）。

### 物理機制（皆附文獻，相關來源已加入文獻資料庫）

- 電性：漸變通道近似 square-law 模型（線性區 + 夾止飽和區）與次臨界指數區 SS = n(kT/q)ln10（Sze & Ng 2007）；Schottky 接觸熱離子發射上限 J = A*T²exp(−qφB/kT)（Sze & Ng 2007；Allain et al. 2015）；量子電容 Cq 與 Cox 串聯選項（Luryi 1988；Ma & Jena 2015）。
- 能帶排列：`bandAlignment.ts` 實作 Schottky–Mott 規則與 Fermi-level pinning 線性內插模型 φBn = S(Φm−Φ_CNL)+(Φ_CNL−χ)，能帶預覽可選接觸金屬並計算 φBn / φBp（Das et al. 2013；Kim et al. 2017）。
- 氧化：新增 Deal-Grove 線性-拋物線律 x²+Ax=B(t+τ)（Deal & Grove 1965）與 Cabrera-Mott 低溫薄膜反對數自限律（Cabrera & Mott 1949），與原線性律並列可選。
- 擴散：新增定濃度表面源 erfc 剖面（Fick 第二定律解析解，Crank 1975），與瞬時源高斯剖面並列可選。
- `constants.ts` 補齊 CODATA 物理常數與 Richardson 常數。

### 版面

- 左右側欄（目前以占位提示為主）與底部圖表面板改為預設收合，點擊展開後記住偏好。
- 電性「進階物理機制」參數收納於可展開區塊，預設值即可運作。
- 氧化成長律參數依選擇的成長律動態顯示，避免無關欄位干擾。

### 測試

- 新增 `npm run test`（Vitest）：物理函數單元測試（17 項數值驗證）與整合測試（所有分頁渲染、預設收合、hash 導覽、成長律切換、進階區塊展開，16 項）。
